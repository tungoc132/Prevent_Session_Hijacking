...
def home():
    ...
    if (session.get('id', None) is None) or not MS.tCheckKey(session.get("id")):
    ...
    return render_template("home.html", user = User.query.filter_by(id = MS.GetDecipher(session.get('id'))).first())
...
def delete_note():
    ...
    if note:
        if note.user_id == MS.GetDecipher(session.get('id')):
